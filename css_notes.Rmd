...
> Now try changing the **div** element to a **section** element instead (this tag is more suitable). Remember to change both the opening and closing tags. Your `.special` class style declaration will still work because we didn't target a specific element.

### Combining class and element selectors

You can combine class selectors with element selectors like this: `div.special`. The `.` that prefixes the class selector acts to concatenate the two selectors together. This can be useful when you need to override a style that already specifies the element. 

You can also concatenate as many classes as you want, so you can imagine a selector that looks something like this: `.special.sale` or `.special.info`. All the elements with `class="special"` will use the same set of declarations but then extra classes (like `sale` or `info` in this example) can be added to differentiate them. Taking this approach helps you to write less repetitive CSS.

You can give an element multiple classes, just by using spaces as separators in the attribute: `class="special sale"`
...
> Now, try adding a new declaration that adds something different to this block, by targeting the new class in your stylesheet.
...
IDs are often used on body tags so that you can write styles that are specific to particular pages. You can combine ID selectors in all the same ways as classes; the `#` prefix acts as a concatenator too. So, selectors with IDs in might start to look like: `#about .special h2`, or `#signup.special` etc. 
